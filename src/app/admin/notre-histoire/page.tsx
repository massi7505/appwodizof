...
export default function AdminNotrHistoirePage() {
  ...
  return (
    <div className="p-6 max-w-3xl">
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-gray-800 text-white px-4 py-3 rounded-xl shadow-lg text-sm border border-gray-700">{toast}</div>
      )}

      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-white">📖 Notre Histoire</h1>
          <p className="text-gray-500 text-sm mt-0.5">Page histoire publique avec sections personnalisables</p>
        </div>
      ...
      <div className="flex gap-2 mb-6">
        {(['settings', 'sections'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${tab === t ? 'bg-amber-500/20 text-amber-400' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
